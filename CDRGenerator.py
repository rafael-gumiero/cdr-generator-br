...
def get_area_codes():
    ...
    f = open("br-area-code-list.csv", "r", encoding="utf-8-sig")
...
# Construct a BR only MSISDN
# Calling country code is always 55
...
# Construct any MSISDN, international allowed but mostly BR
# 95% of the time return a BR MSISDN
# 5% of the time get a valid countro code followed by 10 digits

def get_any_msisdn(areacodes, countrycodes):
    
    if (probability(95)):
        msisdn = get_br_msisdn(areacodes)
    else:
...
def get_cdr(areacodes, countrycodes):
    ...
    calling_msisdn = get_br_msisdn(areacodes)
